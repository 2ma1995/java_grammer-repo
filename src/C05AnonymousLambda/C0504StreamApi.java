...
//        java에서  함수형프로그래밍을 지원을 위한 라이브러리는 streamAPI
...
////        원시자료형을 가지고 stream객체를 만들게 되면, Stram<T>가 아니라,별도의 Stream객체 존재
...
//        filter : 특정기준으로 대상을 filtering 한뒤에 새로운 스트림을 반환.
...
//
//        String[] stArr = {"HTML","CSS","JAVA","PYTHON"};
//        제네릭 타입소거를 인해, .toArray를 통해 바로 새로운 String배열을 만드는 것이 불가
...
//        String[] stArr = {"HTML","CSS","JAVA","PYTHON"} ;
//        IntStream stream1 = Arrays.stream(stArr).mapToInt(a-> a.length());
...
////        arr3에 홀수만 걸뤄서 해당 홀수의 제곱값을 담기
//        int[] arr3 = Arrays.stream(arr1).filter(a-> a%2!=0).map(a-> a*a).toArray();
////        arr4에 홀수만 거르고,제곱값을 구하고, 해당 숫자들을 오름차순한 순서를 담아주세요
...
////        optional객체 : 값이 있을수도 있고, 없을수도 있읆을 명시한 객체
...
//
////        Optional객체 : 특성객체에 값이 없을지도 모른다는것을 명시적으로 표현
...
//        Optional<String > opt3 = Optional.of("hello");//값이 있는 경우만 Optional.of 로 생성가능
//
////        (중요)Optional객체 처리방법 4가지
////        방법1. is{resent()로 확인후에 get()
//        if (opt2.isPresent()){
...
////        방법4. orElseThrow : 값이 있으면 있는 값 return , 없으면 이정된 예외 강제 발생
////        개발에서 사용자에세 적절한 메시지 전달 목적과 의도된 코드중단을 목표로 강제로 예외를 발생시키는 경우가 많음.
...
//        예시1
        ...
        System.out.println(students.stream().mapToInt(a->a.getAge()).average().orElseThrow(()->new NoSuchElementException()));
...
//        Student가 있으면, student출력, 없으면 "해당학생은 없습니다."라고 에러메세지와 함께 에러 강제 발생.
        System.out.println(myStudent.orElseThrow(()->new NoSuchElementException("해당 학생은 없습니더.")));
    }
}
